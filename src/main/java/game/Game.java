...
public class Game {
    ...
    public static void main(String[] args) {
        ...
        ThreeDScatterPanel t = new ThreeDScatterPanel(course,pp);
        try {
            AnalysisLauncher.open(t);
        } catch (Exception e) {
            e.printStackTrace();
        }
        while (true) {
            if (pp.get(currentPlayer).getBall().isMoving) {
                ...
                if (selectNextPlayer){
                    ...
                    pp.get(currentPlayer).getBall().getPhysics().init(course, pp.get(currentPlayer).getBall());
                    dp.setCurrentPlayer(pp.get(currentPlayer));
                    t.setCurrentPlayer(pp.get(currentPlayer));
                    JOptionPane.showMessageDialog(null, "It is " +  pp.get(currentPlayer).getName() + " turn", "Player", JOptionPane.INFORMATION_MESSAGE);
                    dp.repaint();
                    t.update();
                }
    ...
    private static void showScoreOnScreen() {
        ...
        for (int i = 0; i < pp.size(); i++) {
            Player p = pp.get(i);
            scores.append("Player: " + p.getName() + "  Total Strokes: " + p.getTotalStrokes() + " Current Strokes: "  + p.getCurrentStrokes() + "\n");
        }
