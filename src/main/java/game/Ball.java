...
public class Ball {
    ...
    public Coord3d[] getPointsForScatter() {
        ArrayList<Coord3d> c = new ArrayList<Coord3d>((int) (radius*radius*radius));
        for (int i = 0; i < 10; i++) {
            c.add(new Coord3d(coordinate.getX()+i,coordinate.getY()+i,coordinate.getZ()+i));
        }
        Coord3d[] cord = new Coord3d[c.size()];
        c.toArray(cord);
        return (cord);
    }
}